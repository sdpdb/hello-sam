...
var (
  ...
  queryTimeout  = flag.String("query.timeout", "30", "Query timeout (in seconds).")
...
func NewExporter(dsn string) *Exporter {
  return &Exporter{
    ...
    memresize: prometheus.NewGaugeVec(prometheus.GaugeOpts{
      Namespace: namespace,
      Name:      "memresize",
      Help:      "Gauge metric with bytes of all Lobs per Table.",
    }, []string{"database","dbinstance","compname","type"}),
  }
}
...
// ScrapeRecovery collects memory resize metrics
func (e *Exporter) ScrapeMemresize() {
  var (
    rows *sql.Rows
    err  error
  )
  for _, conn := range config.Cfgs {
    if conn.db != nil {
      rows, err = conn.db.Query(`select main.component compname, SUM(case when OPER_TYPE in ('STATIC') then initial_size/1024/1024 when (initial_size-final_size)=0
                                 AND status not in ('COMPLETE','STATIC') then ((initial_size-final_size)+5) else ((initial_size-final_size)*-1)/1024/1024 end) value
                                 from v$sga_resize_ops main  group by main.component`)
      if err != nil {
        continue
      }
      defer rows.Close()
      for rows.Next() {
        var compname string
        var value float64
        if err := rows.Scan(&compname, &value); err != nil {
          break
        }
        e.recovery.WithLabelValues(conn.Database,conn.Instance,compname).Set(value)
      }
    }
  }
}
